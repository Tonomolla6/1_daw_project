...
function finish_cart() {
    $.ajax({ 
        type: 'GET',
        url: "module/cart/controller/cart.php?op=money_checking",
        dataType: 'json'
    }).done(function(data) {
        console.log(data);
...
function cart_user_php(stat) {
    ...
    $.ajax({ 
            type: 'GET',
            url: "module/cart/controller/cart.php?op=insert_cart_user",
            dataType: 'json',
            data: {data: products_cart}
        }).done(function(data) {
            draw_cart(stat);
        }).fail(function(textStatus) {
            console.log("Error en la promesa");
    ...
    if (login() != true && login() != false){
        var select_php = function () {
            return new Promise(function(resolve, reject) {
                $.ajax({ 
                        type: 'GET',
                        url: "module/cart/controller/cart.php?op=select_cart_user",
                        dataType: 'json'
                    }).done(function(data) {
                        if (data){
                            var products = new Array();
                            for (let i = 0; i < data.length; i++) {
                                products[i] =  { id: data[i][0], und: parseInt(data[i][1]) };
                            }
